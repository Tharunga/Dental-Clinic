...
<div class="app-shell">
    ...
    <main class="main">
        <div class="topbar">
            <div>
                <h1>Edit appointment ${number}</h1>
                <p>Update patient details, visit schedule, or status when the patient changes plans.</p>
            </div>
        </div>
        ...
        <section class="card">
            <form method="post" action="${pageContext.request.contextPath}/appointments/edit">
                ...
                <div class="form-grid">
                    <div class="field">
                        <label for="nicDisplay">NIC number</label>
                        <input id="nicDisplay" value="${nic}" readonly>
                    </div>
                    <div class="field">
                    ...
                    <div class="field">
                        ...
                        <select id="dentistId" name="dentistId" required>
                            ...
                            <option value="<%= id %>" <%= id.equals(selDentist) ? "selected" : "" %>>
                                <%= d.getDentistName() %> — <%= d.getSpecialization() %><%= d.isActive() ? "" : " (inactive)" %>
                            </option>
                            <% } } %>
                        </select>
                    </div>
                    ...
                    <div class="field">
                        ...
                        <select id="status" name="status" required>
                            ...
                            <option value="<%= s %>" <%= s.equalsIgnoreCase(selStatus) ? "selected" : "" %>><%= s %></option>
                            <% } %>
                        </select>
                    </div>
                </div>
                <div class="actions">
                    ...
                    <a class="btn btn-ghost" href="${pageContext.request.contextPath}/appointments/search?number=${number}">Cancel</a>
                </div>
            </form>
        </section>
    </main>
